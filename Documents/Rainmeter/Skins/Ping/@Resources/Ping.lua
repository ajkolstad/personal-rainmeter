...
function ShowSet()
    flagSet = true
    SKIN:Bang('[!MoveMeter (#GraphW#/4) '..(SKIN:GetMeter('Graph'..setPos):GetY())..' ShiftUp][!SetOption NameSet Text #*Name'..setPos..'*#][!SetOption AddressSet Text #*Address'..setPos..'*#][!SetOption ShortcutSet Text #*Shortcut'..setPos..'*#][!SetOption ThresholdYSet Text #*ThresholdY'..setPos..'*#][!SetOption ThresholdRSet Text #*ThresholdR'..setPos..'*#][!UpdateMeterGroup Set][!ShowMeterGroup Set][!Redraw]')
end
...
function Set2()
    ...
    elseif setVar ~= 'Shortcut' then
        variable[setVar][setPos] = tonumber(val) or val
    end
...
function Swap(n, m)
    local varName = {'Open','Name','Address','Shortcut','ThresholdY','ThresholdR'}
    for i = 1, 6 do
        if i ~= 3 and i ~= 4 then
